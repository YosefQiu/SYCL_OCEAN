...
static inline std::vector<std::string> glob(const std::string &pattern)
{
    ...
    // Extract directory and pattern from the provided path
    fs::path path_pattern(pattern);
    std::string dir = path_pattern.parent_path().string();
    std::string filename_pattern = path_pattern.filename().string();
    ...
    // Iterate over the directory and match filenames with the pattern
